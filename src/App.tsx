...
import Workspace, { readState, storageKey } from "./Workspace";
...
export default function App() {
  ...
  const [saved, setSaved] = useState<string[]>(() =>
    readState()
      .jobs.filter((j) => j.status === "saved")
      .map((j) => j.id),
  );
  const [saveError, setSaveError] = useState("");
  const [saving, setSaving] = useState(false);
  const [openFaq, setOpenFaq] = useState<number | null>(0);
  const [workspace, setWorkspace] = useState<"profile" | "inbox" | null>(null);
  const item = connections[connection];
  const job = initialJobPostings[item.job];
  const start = () => setWorkspace("profile");
  async function saveExample() {
    const current = readState();
    const status = saved.includes(job.id) ? "arrived_new" : "saved";
    setSaveError("");
    setSaving(true);
    try {
      const response = await fetch("/api/ai/tune-hypothesis", {
        method: "POST",
      ...
      const next = {
        ...current,
        jobs: current.jobs.map((j) => (j.id === job.id ? { ...j, status } : j)),
      };
      localStorage.setItem(storageKey, JSON.stringify(next));
      setSaved(next.jobs.filter((j) => j.status === "saved").map((j) => j.id));
    } catch {
      setSaveError(
        "저장하지 못했어요. 연결과 브라우저 저장 설정을 확인한 뒤 다시 시도해주세요.",
      );
    } finally {
      setSaving(false);
    }
    ...
    <>
      ...
      <header className="site-header">
        <div className="nav-shell">
          <a className="wordmark" href="#" aria-label="잡이쓰 홈">
            jobiss<span>.</span>
          </a>
          ...
          <div className="letter-layout">
            <div className="connection-menu">
              ...
              >
                {connections.map((c, i) => (
                  <button
                    ...
                    className={`connection-tab ${connection === i ? "active" : ""}`}
                    onClick={() => {
                      setConnection(i);
                      setReport(false);
                      setSaveError("");
              ...
              <div className="sample-note">
                <span className="sample-avatar">민</span>
                <p>
                  <strong>민석님의 이야기를 예시로 보여드려요.</strong>
                  <br />
                  나의 경험을 등록하면, 나만의 연결이 시작돼요.
                </p>
              </div>
            ...
            >
              <div className="letter-top">
                <span className="letter-to">To. 민석님</span>
              ...
              <p className="letter-greeting">민석님, 이런 연결을 발견했어요.</p>
              ...
              <div className="job-preview">
                <div className={`company-monogram company-${item.job}`}>
                  {job.companyInitial}
                </div>
                <div>
              ...
              <div className="letter-actions">
                ...
                <button
                  className={`save-letter ${saved.includes(job.id) ? "is-saved" : ""}`}
                  aria-label={
      ...
      <footer className="site-footer content-width">
        <div>
          <a href="#" className="wordmark" aria-label="잡이쓰 홈">
            jobiss<span>.</span>
          </a>
